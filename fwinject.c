...
static void print_usage(const char *name)
{
	printf("Usage: %s unpatched_firmware.bin section_filename section_number output_filename\n", name);
}
...
int parse_firmware(unsigned char *buf, int size, struct section_info *output, int max_sections)
{
	int verbose = 0;
	int ret = 0;
...
int main(int argc, char **argv)
{
	...
	int fw_size, replacement_size, num_sections;
